...
import {
  ...
  isCaseStudy, metaRow, metricsBlocks, hasMetrics, noHref,
} from "../shared.js";
...
${sec.projects && p.projects.length ? `<section><div class="wrap"><div class="sec-head"><span class="sec-no mono">03</span><h2>Selected analyses</h2></div>${p.projects.filter(isCaseStudy).map((pr) => { const href = caseHref(pr); const hasDmf = pr.problem || pr.process || pr.results; return href
  ...
  : `<div class="proj-cs">${pr.cover ? `<img class="proj-cover" src="${pr.cover}" alt="${esc(pr.name)}">` : ""}<h3>${esc(pr.name)}</h3>${pr.summary || pr.desc ? `<p class="sum">${esc(pr.summary || pr.desc)}</p>` : ""}${metaRow(pr, "csmeta-r")}${hasDmf ? `<div class="dmf">${pr.problem ? `<div class="dmf-cell"><div class="dmf-label">Dataset</div><p>${esc(pr.problem)}</p></div>` : ""}${pr.process ? `<div class="dmf-cell"><div class="dmf-label">Method</div><p>${esc(pr.process)}</p></div>` : ""}${pr.results ? `<div class="dmf-cell"><div class="dmf-label">Finding</div><p>${esc(pr.results)}</p></div>` : ""}</div>` : ""}${metricsBlocks(pr, "met-row")}</div>`; }).join("")}${p.projects.filter((pr) => !isCaseStudy(pr)).length ? `<div style="margin-top:24px">${p.projects.filter((pr) => !isCaseStudy(pr)).map((pr) => `<div class="proj-simple"><b>${esc(pr.name)}</b><p>${esc(pr.summary || pr.desc)}</p></div>`).join("")}</div>` : ""}</div></section>` : ""}
