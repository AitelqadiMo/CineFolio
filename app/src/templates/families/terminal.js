...
import { esc, indexHead, caseHead, linkRow, credit, normExperience, hasCerts, hasLangObjs, eduLabel, langLabel, slugify, isCaseStudy, noHref, METRICS_CAP, projectLinks } from "../shared.js";
...
${sec.projects && p.projects.length ? `<div><span class="ps">➜ ~</span> <span class="cmd">ls projects/</span></div>${p.projects.map((pr) => { const href = caseHref(pr); const mets = Array.isArray(pr.metrics) ? pr.metrics.slice(0, METRICS_CAP) : []; const metsOut = mets.length ? `<div class="d" style="margin-top:6px"><span style="color:${amber}"># metrics</span><br>${mets.map((m) => { const glyph = { up: "↑", down: "↓" }[m.direction] || ""; return `${esc(m.value)}${glyph ? `<span aria-hidden="true">${glyph}</span>` : ""} ${esc(m.label || "")}`; }).join("<br>")}</div>` : ""; return `<div class="xp"><div class="c">${href ? `<a href="${href}">${esc(pr.name)}/</a>` : `${esc(pr.name)}/`}${isCaseStudy(pr) ? " · case study" : ""}</div><div class="d">${esc(pr.summary || pr.desc)}</div>${href ? `<div class="d" style="margin-top:6px"><a href="${href}">cat projects/${esc(href.replace(/^projects\//, ""))}</a></div>` : `${pr.cover ? `<img src="${pr.cover}" alt="${esc(pr.name)}" style="max-width:100%;border:1px solid ${green}33;border-radius:6px;margin:8px 0">` : ""}${["role","timeline","tools"].filter((k)=>pr[k]).map((k)=>`<div class="d">${k}: ${esc(pr[k])}</div>`).join("")}${["problem","process","results"].filter((k)=>pr[k]).map((k)=>`<div class="d" style="margin-top:6px"><span style="color:${amber}"># ${k}</span><br>${esc(pr[k])}</div>`).join("")}${metsOut}${projectLinks(pr, "plinks")}`}</div>`; }).join("")}` : ""}
...
${["problem", "process", "results"].filter((k) => pr[k]).map((k) => `<div class="blk"><h2>${k === "problem" ? "problem" : k === "process" ? "process" : "results"}</h2><p>${esc(pr[k])}</p></div>`).join("")}${Array.isArray(pr.metrics) && pr.metrics.length ? `<div class="blk"><h2>metrics</h2>${pr.metrics.slice(0, METRICS_CAP).map((m) => { const glyph = { up: "↑", down: "↓" }[m.direction] || ""; return `<div class="d" style="margin-top:4px"><span style="color:${green}">${esc(m.value)}${glyph ? `<span aria-hidden="true">${glyph}</span>` : ""}${projectLinks(pr, "plinks")}</span>  <span style="color:${dim}">${esc(m.label || "")}</span></div>`; }).join("")}</div>` : ""}
