...
// lead, give the logline, trim the acts, and walk out one of two doors, both
// of which YOU drive: The Set (build it live yourself, free forever) or the
// AI Director (send your brief through the AI pipeline). Everything lands in
// ONE dossier (PUT /profile): The Set reads it, the AI pipeline reads it
// (dispatch carries it verbatim), so both engines tell the same story.
// Skippable at every scene, resumable (localStorage draft), and it never
// traps anyone: every exit leads somewhere useful.
...
export default function Onboarding() {
  ...
  const saveDossier = async () => {
    ...
    try { await api.putProfile(profile); track(STEP.profileUploaded); } catch { /* offline: localStorage carries it; Profile syncs later */ }
    return profile;
  };
  ...
  const exitToDirector = async () => {
    setErr("");
    if (cvRaw.trim().length < 80) { setErr("The AI pipeline needs the script. Go back to Scene 1 and drop your resume, or take The Set door."); return; }
    setBusy("director");
    await saveDossier();
    try {
      ...
      if (e.status === 402) {
        ...
        setErr("Your free render is already spent. Take The Set door (free forever), or unlock the Director's Cut from Home.");
      } else setErr(e?.message || "The render hit a snag. Try again, or take The Set door.");
      setBusy("");
    }
  };
  const skip = async () => {
  ...
  const scenes = [
    /* 0 — the script */
    <div key="s0">
      <p className="fs-lead">Every film starts with a script. Drop your resume: the app reads it once and prefills the rest of this walk.</p>
    ...
    <div key="s1" onKeyDown={onEnter}>
      ...
      <div className="fs-photorow">
        ...
        <button type="button" className="fs-ghost" onClick={() => photoRef.current?.click()} disabled={busy === "photo"}>
          {busy === "photo" ? "Developing…" : draft.photo ? "Swap the headshot" : "Add a headshot (the AI renders with it)"}
        </button>
      </div>
    </div>,
    ...
    /* 3 — the acts */
    <div key="s3">
      <p className="fs-lead">The acts of the career. Confirm and trim: the app already drafted them from the script.</p>
      {draft.experience.length === 0 && <p className="fs-dim">Nothing parsed yet, so add the roles that matter (two or three carry a film).</p>}
    ...
    /* 7 — the premiere fork */
    <div key="s7">
      <p className="fs-lead">The dossier is in the vault. Two ways to build it, both premiere at <b>yourname.cinefolio.dev</b>.</p>
      <div className="fs-doors">
        <button type="button" className="fs-door" onClick={exitToSet} disabled={!!busy}>
          ...
          <b>The Set</b>
          <i>Build it live yourself. Every template family, renders as you type. Free forever.</i>
  ...
  return (
    <div className="fs-stage" data-scene={scene}>
      <div className="fs-topbar">
        <span className="fs-brand">CineFolio</span>
